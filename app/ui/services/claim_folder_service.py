        ...
        try:
            if stop_cb and stop_cb():
                return ClaimFolderProcessResult(
                    False, None, None, ["⚠️ Scan cancelled."], "Cancelled"
                )
            ...
            if stop_cb and stop_cb():
                return ClaimFolderProcessResult(
                    False, scan_result, None, ["⚠️ Scan cancelled."], "Cancelled"
                )
            ...
            if scan_result.unknown_files:
                ...
                logs.append(
                    "  ℹ️  Tip: rename files to include keywords like chassis, odometer, cheque, caseless, non_caseless, etc."
                )
                    ...
                    try:
                        if is_auto_generated:
                            try:
                                ...
                                logger.warning(
                                    f"Failed to remove old auto-generated Excel/audit: {rm_exc}"
                                )

                        from app.data.assessment_generator import (
                            generate_primary_assessment,
                        )
                        ...
                        if generated_path:
                            ...
                            logs.append(
                                f"🔧 Auto-generated: {Path(generated_path).name} from {Path(scan_result.excel_path).name}"
                            )
                        else:
                            logs.append(
                                "⚠️  Auto-generation skipped: no parts/labour data found in main Excel"
                            )
                    except Exception as gen_exc:
                        logs.append(
                            f"⚠️  Auto-generation of assessment Excel failed: {gen_exc}"
                        )
            ...
            if self.portal_id == "oic" and scan_result.excel_path:
                ...
                try:
                    ...
                    if is_oic_auto_generated:
                        try:
                            old_oic_path = claim.assessment_files[
                                "oic_assessment_excel"
                            ]
                            ...
                            logger.warning(
                                f"Failed to remove old OIC auto-generated Excel/audit: {rm_exc}"
                            )
                    ...
                    if oic_generated_path:
                        ...
                        logs.append(
                            f"🔧 OIC Auto-generated: {Path(oic_generated_path).name} from {Path(scan_result.excel_path).name}"
                        )
                    else:
                        logs.append(
                            "⚠️  OIC auto-generation skipped: no parts/labour data found in main Excel"
                        )
                except Exception as gen_exc:
                    logs.append(
                        f"⚠️  OIC auto-generation of assessment Excel failed: {gen_exc}"
                    )
                ...
                for doc_name, file_path in all_docs.items():
                    if "cheque" in doc_name.lower() or "check" in doc_name.lower():
                        if "fallback" in os.path.basename(file_path).lower():
                            logs.append(
                                f"ℹ️ Cheque file is an invoice fallback ({Path(file_path).name}); skipping OCR to prevent hanging."
                            )
                            continue
        ...
        try:
            with pdfplumber.open(invoice_pdf) as pdf:
                ...
                if not all_text.strip():
                    ...
                    logs.append(
                        "  ⚠️ PDF has no extractable text (scanned image?), trying OCR..."
                    )
    ...
    def _ocr_extract_invoice(
                ...
                for page_idx, page in enumerate(pdf.pages[:3]):  # First 3 pages max
                    ...
                    try:
                        ...
                        if (
                            ...
                            logs.append(
                                "  ⚠️ PDF page rendering needs Ghostscript. Trying direct PaddleOCR on file..."
                            )
                            break
                        logs.append(
                            f"  ⚠️ Page {page_idx + 1} render failed: {str(render_exc)[:80]}"
                        )
        except Exception as exc:
        ...
        if pil_images:
            try:
                ...
                if all_text.strip():
                    ...
                    logs.append(
                        "  ⚠️ PaddleOCR produced no text from scanned PDF pages."
                    )
            except Exception as paddle_exc:
                logs.append(
                    f"  ⚠️ PaddleOCR unavailable ({type(paddle_exc).__name__}: {str(paddle_exc)[:80]}), trying pytesseract..."
                )
        ...
        if not paddle_ok and pil_images:
            try:
                ...
                logs.append(
                    "  ⚠️ Neither PaddleOCR nor pytesseract could extract text — skipping OCR."
                )
                return None, None
                ...
                if (
                    ...
                    logs.append(
                        "  ⚠️ Tesseract binary not installed. Install from https://github.com/UB-Mannheim/tesseract/wiki"
                    )
                else:
        ...
        if not all_text.strip():
            if not pil_images:
                logs.append(
                    "  ⚠️ Could not render PDF pages for OCR — no images produced."
                )
            return None, None
