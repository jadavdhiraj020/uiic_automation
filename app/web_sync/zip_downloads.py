...
from .manifest import integrity, latest_corrected, plan_local_files
...
def _request_zip(client, case_id, dispatch_id, destination, progress):
    ...
    for attempt in range(1, 4):
        ...
        try:
            _notify(progress, kind="zip_start", message=f"Requesting case ZIP (attempt {attempt} of 3)")
            client.call(
                ZIP_FUNCTION, body, destination=destination,
                max_bytes=MAX_COMPRESSED_BYTES,
            )
            if destination.stat().st_size > MAX_COMPRESSED_BYTES:
                raise DownloadLimitError("ZIP exceeds the 25 MiB compressed safety limit")
            return
            ...
            _notify(
                ...
                message=f"ZIP attempt {attempt} of 3 failed: {exc}. Retrying in {wait_seconds} seconds",
            )
...
def _download_zip_case(client, case_id, dispatch_id, folder, progress):
    ...
    try:
        ...
        _notify(
            ...
            message=f"ZIP staging complete: {len(mapping)} file(s) verified in {elapsed:.1f} seconds; latest Excel: {latest_excel}",
        )
        return latest_excel
    finally:
...
def download_case_preferred(client, case_id, folder, progress=None, automation_dispatch_id=None):
    ...
    except (ZipIdentityError, UnsafeZipError):
        raise
    except ApiError as exc:
